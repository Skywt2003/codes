...
inline void Prim(){
	memset_clear_max(low);
	low[1]=-1;
	for (int i=2;i<=n;i++) low[i]=gcd(a[i],a[1]);
	for (int i=1;i<n;i++){
		int now_max=-INF,k=-1;
		for (int i=1;i<=n;i++) if (low[i]!=-1&&low[i]>now_max) now_max=low[i],k=i;
		ans+=now_max;
		low[k]=-1;
		for (int j=1;j<=n;j++) if (low[j]!=-1) low[j]=max(low[j],gcd(a[k],a[j]));
	}
}
