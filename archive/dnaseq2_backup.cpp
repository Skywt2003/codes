...
inline int char_to_num(char ch){
	return ch=='A'?0:(ch=='C'?1:(ch=='G'?2:(ch=='T'?3:-1)));
}

inline char num_to_char(int num){
	return num==0?'A':(num==1?'C':(num==2?'G':(num==3?'T':'?')));
}
...
struct string_num{
	...
	void operator =(string s){
		...
		for (int i=0;i<len;i+=16){
			a[0]++;
			for (int j=i;j<min(i+16,len);j++){
				a[a[0]]=a[a[0]]*4+char_to_num(s[j]);
			}
		}
	}
	string tostr()const{
		...
		for (int i=1;i<=tmp[0];i++){
			now.clear();
			while (tmp[i]) now.push_back(tmp[i]%4),tmp[i]/=4;
